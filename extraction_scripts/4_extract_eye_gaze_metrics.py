...
# This will now work because we added REPO_ROOT to sys.path
try:
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="Max number of ET frames to process (default: all)",
    )
...
def main():
    ...
    max_et = args.max_frames if args.max_frames else eye_frame_count
    total_to_process = min(max_et, eye_frame_count) + rgb_frame_count
    progress_bar = tqdm(total=total_to_process)
    value_mapping = {}
    et_frames_processed = 0
    for data in provider.deliver_queued_sensor_data(deliver_option):
        ...
        if data.sensor_data_type() == SensorDataType.IMAGE:
            ...
            if data.stream_id() == eye_stream_id:
                et_frames_processed += 1
                if args.max_frames and et_frames_processed > args.max_frames:
                    continue
